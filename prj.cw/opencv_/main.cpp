...
struct Ñircuit {
	Ñircuit(int ar, cv::Rect st, cv::Rect en) : area(ar), start(st), end(en) {}
	~Ñircuit() = default;
...
int main() {
	cv::VideoCapture cap(0); // 0 - èíäåêñ êàìåðû
	...
	// ñ÷èòûâàíèå êàäðà ñ êàìåðû
	cv::Mat frame;
	cap >> frame;

	// ñîõðàíåíèå êàäðà â ôàéë
